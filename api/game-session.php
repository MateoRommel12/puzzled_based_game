...
try {
    ...
    if (isset($_GET['debug'])) {
        error_log("Game Session API Debug - Action: $action, Method: $method");
    ...
    error_log("Game Session API Error: " . $e->getMessage());
    error_log("Error occurred in action: " . ($action ?? 'unknown'));
...
function handleSaveGameSession($db) {
    ...
    $stmt = $db->prepare("
        INSERT INTO game_sessions (
            user_id, game_type, score, difficulty_level, time_taken,
    ...
    $stmt->execute([
        $userId, $gameType, $score, $difficulty, $timeTaken,
